...
MainWindow::MainWindow() : MainWindowUI() {
  ...
  m_logview_in->callback(
      [](Fl_Widget *w, void *v) { ((LogView *)v)->zoom(0.2); }, m_logview);
  m_logview_out->callback(
      [](Fl_Widget *w, void *v) { ((LogView *)v)->zoom(-0.2); }, m_logview);
  m_logview_back->callback(
      [](Fl_Widget *w, void *v) {
        ((LogView *)v)->shift(std::chrono::seconds{-5});
      },
      m_logview);
  m_logview_forward->callback(
      [](Fl_Widget *w, void *v) {
        ((LogView *)v)->shift(std::chrono::seconds{5});
      },
      m_logview);
...
void MainWindow::update_log(std::optional<std::shared_ptr<Log>> l) {
  if (l) {
    ...
    auto start_time = stop_time - std::chrono::seconds{1800};
